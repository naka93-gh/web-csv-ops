...
```ts
import { parse, defineSchema } from "web-csv-ops";
...
- **`defineSchema` で定義すると `data` の各列が型付き**（`age: number | null` 等）になる。`{...} satisfies Schema` でも動くが、`prop` が widen して静的な型は緩くなる（runtime はどちらも同じ）
...
| `defineSchema(schema)`                                   | スキーマ定義（型付け用にリテラル保持） |
...
- parse: 型付き出力の静的型を効かせるには `defineSchema` で定義する（`satisfies Schema` は `prop` が widen し型が緩くなる。runtime は同じ）
